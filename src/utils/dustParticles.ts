...
export const createDustParticles = (container: HTMLElement | null, count: number = 15, particleColor: string = "#FFD700") => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    dust.style.left = `${Math.random() * container.clientWidth}px`;
    dust.style.top = `${Math.random() * container.clientHeight}px`;
    ...
    gsap.to(dust, {
      x: () => (Math.random() - 0.5) * (container.clientWidth / 5),
      y: () => (Math.random() - 0.5) * (container.clientHeight / 5),
      opacity: 0,
      ...
      onRepeat: () => {
        gsap.set(dust, {
          x: (Math.random() - 0.5) * (container.clientWidth / 10),
          y: (Math.random() - 0.5) * (container.clientHeight / 10),
